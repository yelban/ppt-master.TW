...
**Mandatory**: write one concise, non-empty, executable `Layout pattern` per non-formula row in ordinary language; optional ids from [`image-layout-patterns.md`](./image-layout-patterns.md) must be exact.

**Default — action-bearing image composition (may override when deliberate restraint is documented for the page)**: For a `page_role: hero_page` or otherwise image-led row, name a concrete image/content or image/shape relationship. Position, size, routine crop, and a legibility-only scrim alone do not constitute that relationship. A plain split or full-bleed treatment remains valid only when page-specific restraint is deliberate; state why in the matching §IX `Layout` / `Images`.
