...
**Mandatory**: each placed row, including formulas, gets one executable `Layout pattern`. It is preferred expression, not locked geometry; optional ids from the already-read [`image-layout-patterns.md`](./image-layout-patterns.md) must be exact. Executor may adapt it while preserving resource identity/source, must-use status, crop/content, and explicit user/template constraints; layout-only changes need no upstream rewrite.
