...
- Use PowerPoint-installed exported faces. Safe anchors: CJK `Microsoft JhengHei` / `Microsoft JhengHei` / `PMingLiU` / `FangSong` / `KaiTi`; Latin sans `Arial` / `Calibri` / `Segoe UI` / `Verdana` / `Trebuchet MS`; Latin serif `Times New Roman` / `Georgia` / `Cambria` / `Garamond` / `Book Antiqua`; mono `Consolas` / `Courier New`; display `Impact` / `Arial Black`. Other export-safe families are limited to sparse short display/ornament; structural or recurring use returns upstream.
...
- Avoid splitting roles across near-equivalents such as YaHei↔PingFang, PMingLiU↔Songti, Arial↔Helvetica↔Segoe UI, or Times New Roman↔Times. A cross-platform counterpart may remain inside one fallback stack.
