...
**SVG 作為嵌入圖片** 是最簡單的路線——把整張幻燈片渲染成圖片塞進 PPT。但這樣完全喪失可編輯性，形狀變成像素，文字無法選中，顏色無法修改，和截圖沒有本質區別。
...
| 圖片事實來自重算後設資料 | `analysis/image_analysis.csv` 從即時 `images/` 目錄重算；Strategist 先用源文上下文，只在圖片語義或安全放置仍無法確定時檢視那一張具體圖片；Executor 不重新讀取源影像素 |
...
值得標註的架構選擇：**viewBox 是像素，不是絕對單位。** 像素空間讓 AI Executor 思考佈局沒有歧義（`x="100"` 就是左緣 +100px），人類在瀏覽器裡檢查也直接。到 EMU 的換算只在匯出時發生一次——選像素意味著流水線的其餘環節（Strategist、Executor、質量檢查、後處理）永遠不需要在 EMU 思維下工作，那對 AI 生成和人類除錯都是敵對的。
...
`update_spec.py` 用兩個協調步驟傳播一次有意的 deck 級錨點修改：先規劃並字面替換每一份 `svg_output/*.svg`，這些 SVG 更新成功後才寫權威 `spec_lock.md`。工具的範圍**故意收得很窄**——只支援 `colors.*`（HEX 值，大小寫不敏感替換）和全域性 `typography.font_family`（屬性級）。全域性字型替換還會在一次 lock 檔案寫入中，把所有既有 `typography.*_family` 行同步為同一值，避免標題、正文或其他角色保留已從全部 SVG 移除的舊字型。其他欄位（字號、按角色字型變更、圖示、圖片、畫布）**有意不支援**——它們的替換需要屬性級或語義級理解，風險/收益不值得做批次傳播。當重複出現的上下文值被明確提升為具名語義角色時，反向回寫 lock 同樣合理；但不能只為了清空 checker 的資訊提示而擴充 lock。其他欄位應修改其權威產物並重做受影響頁面。
...
「圖片**怎麼放上幻燈片**」的詞表（完整詞彙在 [`references/image-layout-patterns.md`](../../skills/ppt-master/references/image-layout-patterns.md)）把 99 條穩定編號技法拆成兩層、自由組合：
